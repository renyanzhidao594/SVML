...
if st.button("Predict Liver Metastasis Risk"):
    ...
    # 展示预测结果（优化概率展示逻辑，避免类别顺序歧义）
    st.write("### Prediction Result")
    # 明确获取无转移/有转移的概率
    no_metastasis_prob = predicted_proba[list(model.classes_).index(0)] * 100
    metastasis_prob = predicted_proba[list(model.classes_).index(1)] * 100
    ...
    # 展示参考阈值
    st.caption(f"Reference Threshold: {svm_best_thresh:.2f} (Probability ≥ this value = High Risk)")
